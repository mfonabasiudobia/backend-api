...
let fast2smsSecretKey = config.get('general.fast2smsKey');
// let phoneNumber=undefined;
const storage = multer.memoryStorage();
const upload = multer({ storage });
const fast2smsKey = fast2smsSecretKey;
...
router.post("/login", async(req, res) => {
    try {
        ...
        if (user != null) {
            ...
            try {
                const smsResult = await sendOTP(Phone, newSecret.token);
                if (smsResult.success) {
                    console.log(`✅ OTP sent successfully to ${Phone} via Twilio`);
                } else {
                    console.error(`❌ Failed to send OTP via Twilio: ${smsResult.error}`);
                    ...
                    https.get(`https://www.fast2sms.com/dev/bulkV2?authorization=${fast2smsKey}&route=otp&sender_id=WPISMS&message=148262&variables_values=${newSecret.token}&flash=0&numbers=${Phone}`, (resp) => {
                        console.log("Fallback SMS sent via Fast2SMS");
                    });
                }
            } catch (smsError) {
            ...
            // Send OTP via Twilio SMS for new user
            try {
                const smsResult = await sendOTP(Phone, newSecret.token);
                if (smsResult.success) {
                    console.log(`✅ OTP sent successfully to new user ${Phone} via Twilio`);
                } else {
                    console.error(`❌ Failed to send OTP via Twilio: ${smsResult.error}`);
                    // Fallback to Fast2SMS if Twilio fails
                    https.get(`https://www.fast2sms.com/dev/bulkV2?authorization=${fast2smsKey}&route=otp&sender_id=WPISMS&message=148262&variables_values=${newSecret.token}&flash=0&numbers=${Phone}`, (resp) => {
...
router.post("/login/admin", async(req, res) => {
    ...
    try {
        ...
        try {
            const smsResult = await sendOTP(phone, newSecret.token);
            if (smsResult.success) {
                console.log(`✅ Admin OTP sent successfully to ${phone} via Twilio`);
            } else {
                console.error(`❌ Failed to send admin OTP via Twilio: ${smsResult.error}`);
                // Fallback to Fast2SMS if Twilio fails
                https.get(`https://www.fast2sms.com/dev/bulkV2?authorization=${fast2smsKey}&route=otp&sender_id=WPISMS&message=148262&variables_values=${newSecret.token}&flash=0&numbers=${phone}`, (resp) => {
                    console.log("Fallback admin SMS sent via Fast2SMS");
                });
            }
        } catch (smsError) {
